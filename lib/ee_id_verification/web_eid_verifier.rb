  ...
  class WebEidVerifier
    ...
    def verify_auth_token(auth_token, challenge_nonce)
      ...
      begin
        ...
        if auth_token["signature"]&.start_with?("mock-signature-")
          return handle_mock_authentication(auth_token)
        end
    ...
    def validate_token_structure(token)
      ...
      unless token["format"].start_with?("web-eid:")
        raise Error, "Invalid token format: #{token['format']}"
      end
      
      unless valid_algorithm?(token["algorithm"])
    ...
    def verify_signature(token, challenge_nonce)
      ...
      when /^ES/
        # ECDSA signature
        unless public_key.verify(digest_algorithm, signature, signed_data)
      ...
      when /^[PR]S/
        ...
        unless public_key.verify(digest_algorithm, signature, signed_data, padding)
          raise Error, "Invalid signature"
        end
      else
