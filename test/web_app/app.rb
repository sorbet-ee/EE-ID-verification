...
class WebEidTestApp < Sinatra::Base
  ...
  post "/api/auth/login" do
    ...
    halt 400, { error: "No challenge nonce found" }.to_json unless stored_nonce && nonce_created_at

    # Check nonce expiration (5 minutes)
    halt 400, { error: "Challenge nonce expired" }.to_json if Time.now.to_i - nonce_created_at > 300
